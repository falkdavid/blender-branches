...
static void heap_down(Heap *heap, uint i, HeapComparatorFP cmp)
{
...
static void heap_up(Heap *heap, uint i, HeapComparatorFP cmp)
{
  HeapNode **const tree = heap->tree;
...
static bool heap_is_minheap(const Heap *heap, uint root, HeapComparatorFP cmp)
{
  if (root < heap->size) {
